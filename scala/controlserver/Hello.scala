...
object Hello {
  ...
  def startServer(socket: Socket):Unit = {
    ...
    loop.printWelcome()
    loop.interpret("import org.apache.kafka.clients.admin._")
    val bb = loop.bind("A", "org.apache.kafka.clients.admin.AdminClient", getAdmin())
    print(bb)
    loop.process(new Settings())
    socket.close()
    }

  def main(args: Array[String]):Unit = print("hi")
